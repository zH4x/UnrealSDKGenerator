...
bool Package::DumpFunctions(const fs::path & path) const
{
	std::ofstream o(path / "FunctionsDump.txt", std::ostream::out | std::ostream::app);

	for (auto& curClass : classes)
	{
		for (auto& curMethod : curClass.Methods)
		{
			if (!curMethod.IsNative) continue;

			auto fnName = tfm::format("%s::exec%s", curClass.NameCpp, curMethod.Name);

			tfm::format(o, "0x%p , %-100s \"%s\" \n", curMethod.NativeFunc, fnName, BuildMethodSignature(curMethod, curClass, false));
		}
	}
...
void Package::GenerateClass(const UEClass& classObj)
{
	...
	{
		static auto ptr = UObject::FindObject<UClass>(%s);
		return ptr;
...
void Package::GenerateMethods(const UEClass& classObj, std::vector<Method>& methods) const
{
	...
	for (auto prop = classObj.GetChildren().Cast<UEProperty>(); prop.IsValid(); prop = prop.GetNext().Cast<UEProperty>())
	{
		if (prop.IsA<UEFunction>())
		{
			...
			if (m.IsNative)
			{
				m.NativeFunc = function.GetNative();
			}
...
void Package::SaveFunctions(const fs::path& path) const
{
	...
	{
		PrintFileHeader(os, { GenerateFileName(FileContentType::Classes, *this) }, false);
	}
